...
def controller():
    while True:
        ...
        if c == "f":
            if get_fingerprint():
                if(finger.finger_id==1 or finger.finger_id==2 or finger.finger_id==3 or finger.finger_id==4 or finger.finger_id==5):
                    finger.finger_id=123456789102
                    hs=str(finger.finger_id).encode()
                    hs1=hashlib.sha256(hs).hexdigest()
                    print("Aadhar No: ",finger.finger_id)
                    m1=str(finger.finger_id)+"\n"+str(hs1)
                    clientsocket.send(bytes(m1, "utf-8"))
                    print("Detected #: ", str(hs1))
                elif(finger.finger_id==6 or finger.finger_id==7 or finger.finger_id==8 or finger.finger_id==9 or finger.finger_id==10):
                    finger.finger_id=2
                    hs=str(finger.finger_id).encode()
                    hs1=hashlib.sha256(hs).hexdigest()
                    print("Detected #", str(hs1), "with confidence", finger.confidence)
            else:
